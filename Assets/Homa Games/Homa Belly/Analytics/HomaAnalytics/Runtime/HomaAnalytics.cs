...
namespace HomaGames.HomaBelly
{
    public class HomaAnalytics : IDisposable
    {
        ...
        {
            HomaAnalyticsLogListener.Initialize(OnLogReceived);
            
            HomaAnalyticsLogger.Log($"Pre-Initialized. Events will be captured and sent later on initialization.");
        ...
        public void Initialize(HomaAnalyticsOptions overrideOptionsUsedInConstructor = null)
        {
            if (m_initialized)
            {
                HomaAnalyticsLogger.LogWarning("Analytics already initialized.");
                return;
            }
            ...
            m_eventSlotManager = new EventSlotManager(m_eventDispatcher,
                m_analyticsOptions.EventCacheDirectory,
                SLOT_MAX_EVENTS,
                SLOT_TIME_TO_AUTO_DISPATCH_MS,
                MAX_SIZE_IN_BYTES,
                m_analyticsOptions.VerboseLogs);
            ...
            HomaAnalyticsLogger.Log($"Initialized. Install Id: {GetInstallId()} Session Id: {GetSessionId()} NTestingId: {m_analyticsOptions.NTestingId} NTestingOverrideId: {m_analyticsOptions.NTestingOverrideId}");

            if (m_analyticsOptions.VerboseLogs)
            {
                HomaAnalyticsLogger.Log($"Pending Events: {m_preInitializationEvents.Count}");
                HomaAnalyticsLogger.Log($"Options: {m_analyticsOptions}");
            }
        }

        private void OnLogReceived(string condition, string stacktrace, LogType type)
        {
            if (m_analyticsOptions == null)
            {
                return;
            }
        ...
        private bool RecoverOrGenerateInstallId()
        {
            ...
            if (string.IsNullOrWhiteSpace(m_installId))
            {
                ...
                if (m_analyticsOptions.VerboseLogs)
                {
                    HomaAnalyticsLogger.Log($"New install id generated: {m_installId}");
                }
        ...
        public void ToggleAnalytics(bool toggle)
        {
            ...
            {
                HomaAnalyticsLogger.Log($"Toggled: {toggle}");
            }
        ...
        private void CheckNewSession()
        {
               ...
               || elapsedSeconds > m_analyticsOptions.SecondsToGenerateNewSessionId)
            {
                ...
                if (m_analyticsOptions.VerboseLogs)
                {
                    HomaAnalyticsLogger.Log($"New session id set: {m_sessionId}");
                }
        ...
        public void Dispose()
        {
            ...
            m_eventSlotManager?.CancelOnGoingOperations();
            HomaAnalyticsLogListener.Unsubscribe(OnLogReceived);
#if UNITY_EDITOR
            if (Application.isPlaying)
                GameObject.Destroy(m_proxyGameObject);
            else
                GameObject.DestroyImmediate(m_proxyGameObject);
#else
            GameObject.Destroy(m_proxyGameObject);
#endif
        }
        ...
        private async Task DoTrackEventAsync(RuntimeAnalyticsEvent runtimeAnalyticsEvent)
        {
            ...
            {
                HomaAnalyticsLogger.LogWarning("Can't track null events.");
                return;
            }
            ...
            if (m_analyticsOptions.VerboseLogs)
            {
                HomaAnalyticsLogger.Log($"Tracking: {runtimeAnalyticsEvent.EventName}: {runtimeAnalyticsEvent}");
            }
        ...
        private async Task StoreEventBeforeDispatching(RuntimeAnalyticsEvent analyticsEvent)
        {
            ...
            Task<string> serializeTask = SerializeEventAsync(analyticsEvent);
            await serializeTask;

            var bodyAsJsonString = serializeTask.Result;
            if (bodyAsJsonString == null)
            {
                HomaAnalyticsLogger.LogWarning($"EventValue is null");
                return;
            }
        ...
        private async Task<string> SerializeEventAsync(RuntimeAnalyticsEvent analyticsEvent)
        {
            ...
            await Task.Run(delegate
            {
                ...
                {
                    HomaAnalyticsLogger.LogError($"EventDispatcher: {e}");
                }
            });
        ...
        private bool IsEventValid(RuntimeAnalyticsEvent runtimeAnalyticsEvent)
        {
            ...
            {
                HomaAnalyticsLogger.LogWarning($"Event install id: {runtimeAnalyticsEvent.InstallId} is different to the current install id: {GetInstallId()}. This only can happen if the game data was partially deleted.");
            }
            
            if (string.IsNullOrWhiteSpace(runtimeAnalyticsEvent.EventName))
            {
                HomaAnalyticsLogger.LogError("EventName must be set.");
                return false;
            }

            if (string.IsNullOrWhiteSpace(runtimeAnalyticsEvent.InstallId))
            {
                HomaAnalyticsLogger.LogError($"InstallId can't be null or empty in event: {runtimeAnalyticsEvent.EventName}");
                return false;
            }
            
            if (string.IsNullOrWhiteSpace(runtimeAnalyticsEvent.SessionId))
            {
                HomaAnalyticsLogger.LogError($"SessionId can't be null or empty in event: {runtimeAnalyticsEvent.EventName}");
                return false;
            }

            if (runtimeAnalyticsEvent.EventName.Length > MAX_EVENT_NAME_LENGTH)
            {
                HomaAnalyticsLogger.LogError($"EventName can't have a length major than {MAX_EVENT_NAME_LENGTH} characters.");
                return false;
            }
        ...
        {
            HomaAnalyticsLogger.Log($"Event {runtimeAnalyticsEvent.EventName} will be sent after HomaAnalytics was initialized.");
        ...
        public void ClearPersistentData()
        {
            ...
            if (System.IO.Directory.Exists(m_analyticsOptions.EventCacheDirectory))
            {
                System.IO.Directory.Delete(m_analyticsOptions.EventCacheDirectory,true);
            }
