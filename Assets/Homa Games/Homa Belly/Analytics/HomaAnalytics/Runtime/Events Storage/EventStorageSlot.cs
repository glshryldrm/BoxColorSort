...
{
    public class EventStorageSlot : IDisposable
    {
        ...
        // TODO: Any refactor to this class would benefit from removing `State` from its interface
        // It makes the class hard to test and maintain, this is part of the bigger problem which is
        // the fact that this class breaks the single-responsibility principle
            ...
            bool autoDispatchIfLimitReached = false)
        {
            ...
            if (maxSize == 0)
            {
                HomaAnalyticsLogger.LogError($"Can't setup a slot with size 0. Default size will be used: {DEFAULT_SLOT_SIZE}");
        ...
        /// <summary>
        /// This method is the same as DispatchSlot, but it will wait until all events are dispatched.
        /// Note that playing with internet connection, will make this method to wait forever, so use it carefully.
        /// </summary>
        public async Task DispatchSlotAndWaitsForAllEventsToBeSentAsync(CancellationToken cancellationToken = default)
        {
            await DispatchSlot(cancellationToken);
            
            while (State != EventStorageSlotStates.AllEventsDispatched)
            {
        ...
        /// <summary>
        /// All events will be added to a queue to be dispatched to the server and the slot will be closed.
        ...
        /// </summary>
        public async Task DispatchSlot(CancellationToken cancellationToken = default)
        {
            lock (m_dispatchCalledLock)
            {
                if (m_dispatchCalled)
                {
                    HomaAnalyticsLogger.LogWarning($"[WARNING] Can't dispatch the slot twice: {this}.");
                    return;
                }
            ...
            {
                HomaAnalyticsLogger.LogWarning($"[WARNING] Can't dispatch an empty slot: {this}. Slot will be removed");
        ...
        public bool BypassFileSystem()
        {
            ...
            {
                HomaAnalyticsLogger.LogError($"Can't bypass filesystem in slot {SlotName} because the slot is using it: {State}");
                return false;
            }
        ...
        public async Task<bool> ReadEventsAndInitializeSlot(bool moveToNextStateAutomatically)
        {
            ...
            try
            {
                ...
                while (m_stream.Position < m_stream.Length)
                {
                    ...
                    {
                        HomaAnalyticsLogger.LogError($"End of the event {eventName} is invalid. The file is corrupted: {this}");
                        ClearStream();
                        return false;
                    }
                    SizeInBytes += totalLength;
                    var pendingEvent = new PendingEvent(eventName, eventId, eventBody);
                    AddSavedEvent(false,pendingEvent);
                }
            }
            catch (Exception exception)
            {
                HomaAnalyticsLogger.LogWarning($"[ERROR] Failed to read events from slot file: {exception}");
            }
            finally
            {
                ChangeState(EventStorageSlotStates.Idle,moveToNextStateAutomatically);
            }
        ...
        private void WritePendingEventsIfIdle()
        {
            ...
            HomaAnalyticsTaskUtils.Consume(Task.Delay(1).ContinueWithOnMainThread(async _ =>
            {
                ...
                lock (m_eventsToSaveLock)
                {
                    ...
                    {
                        HomaAnalyticsLogger.LogWarning("[WARNING] Events overflow. We received more events than the max allowed: " + TotalEventsCount()+" VS "+m_maxEvents);
                    }
                }
                try
                {
                ...
                catch (ObjectDisposedException objectDisposedException)
                {
                    ...
                    HomaAnalyticsLogger.LogWarning($"[WARNING] The slot was closed while it had pending info to write, this shouldn't happen very often. {ToString()} {objectDisposedException}");
                }
                catch (Exception exception)
                {
                    HomaAnalyticsLogger.LogError($"Can't save slot: {SlotName}:{State} in the file system. They will be stored in memory instead {exception}. ");
                    lock (m_fileStreamLock)
                    {
                        m_stream = CreateMemoryStream();
                    }
                ...
                {
                    AddSavedEvent(true,eventsBatchToSave.ToArray());
        ...
        private async Task<string> ReadFieldFromFile(int maxFieldSize)
        {
            ...
            {
                HomaAnalyticsLogger.LogError($"Event field size is invalid: {eventFieldSize} Max {maxFieldSize}: . Aborting slot initialization. The file may be corrupted");
                return null;
            }
            ...
            if (string.IsNullOrEmpty(field))
            {
                HomaAnalyticsLogger.LogError(
                    $"Event field is invalid: {field} this slot will be discarded. Aborting slot initialization. The file may be corrupted");
                return null;
            }
        ...
        private void ChangeState(EventStorageSlotStates newState,bool processSlot = true)
        {
            lock (m_stateLock)
            {
                if (newState == State)
                {
                    HomaAnalyticsLogger.LogWarning($"[WARNING] You can't change to the same state: {newState} {SlotName}");
                    return;
                }
            ...
            if (State == EventStorageSlotStates.Idle)
            {
                         ...
                         && !m_dispatchCalled)
                {
                    ...
                    HomaAnalyticsTaskUtils.Consume(DispatchSlot(m_disposeCancellationTokenSource.Token));
                }
            }
        }
        ...
        private async Task TryToDisposeStreamWhenClosed()
        {
            ...
            if (!m_closed)
            {
                HomaAnalyticsLogger.LogError("Can't dispose a non closed slot.");
                return;
            }
            ...
            try
            {
                ...
                // ReSharper disable once MethodHasAsyncOverload, we want to be retro compatible with 2020
                m_stream.Dispose();
            }
            catch (IOException ioException)
            {
                HomaAnalyticsLogger.LogWarning($"Error disposing the slot: {ioException}");
            }
            catch (Exception exception)
            {
                HomaAnalyticsLogger.LogError($"Error disposing the slot: {exception}");
            }
        ...
        private void AddSavedEvent(bool triggerAutoDispatch,params PendingEvent[] pendingEvents)
        {
            lock (m_savedEventsLock)
            {
            ...
            if (triggerAutoDispatch && !m_autoDispatchByTimeTriggered)
            {
                m_autoDispatchByTimeTriggered = true;
                if (!m_dispatchCalled)
                {
                    DelayedDispatchSlot(m_millisecondsToAutoDispatch, m_disposeCancellationTokenSource.Token);
                }
            }
        }
        ...
        private static Stream CreateFileStream(string path, bool read = false)
        {
            ...
            {
                HomaAnalyticsLogger.LogError($"Can't create the file stream for the slot. {e}");
        ...
        private void DelayedDispatchSlot(int milliseconds, CancellationToken cancellationToken)
        {
            HomaAnalyticsTaskUtils.Consume(Task.Delay(milliseconds, cancellationToken).ContinueWithOnMainThread(async _ =>
            {
                if (!m_dispatchCalled)
                {
                    await DispatchSlot(cancellationToken);
