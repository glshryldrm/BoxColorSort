...
namespace HomaGames.HomaBelly
{
    public class GameAnalyticsPostprocessor : AssetPostprocessor
    {
        ...
        {
            // If we receive values from configuration data (key exists and is not empty), use those
            if (configurationData.ContainsKey(jsonFieldKey) && !string.IsNullOrWhiteSpace(configurationData[jsonFieldKey]))
            {
                try
                {
