...
{
    public class GameAnalyticsImplementation : AnalyticsBase, ICustomDimensions
    {
        ...
        public override void TrackEvent(AnalyticsEvent analyticsEvent)
        {
            ExecuteAfterInitialisation(() =>
            {
                ...
                {
                    case ItemConsumed itemConsumed:
                        TrackResourceEvent(ResourceFlowType.Sink, "item", 0, null, itemConsumed.ItemId);
                        break;
                    case ItemObtained itemObtained:
                        TrackResourceEvent(ResourceFlowType.Source, "item", 0, null, itemObtained.ItemId);
                        break;
                    case ResourceFlow resourceFlow:
                        TrackResourceEvent(resourceFlow.FlowType, resourceFlow.Currency, resourceFlow.FlowAmount, resourceFlow.ItemType, resourceFlow.ItemId);
                        break;
                    case LevelStarted levelStarted:
                        TrackProgressionEvent(ProgressionStatus.Start, $"Level_{levelStarted.LevelId}");
                        break;
                    case LevelCompleted levelCompleted:
                        TrackProgressionEvent(ProgressionStatus.Complete, $"Level_{levelCompleted.LevelId}");
                        break;
                    case LevelFailed levelFailed:
                        TrackProgressionEvent(ProgressionStatus.Fail, $"Level_{levelFailed.LevelId}");
                        break;
                    case LevelFirstCompletion levelFirstCompletion:
                        TrackDesignEvent($"Levels:Duration:{levelFirstCompletion.LevelId}", levelFirstCompletion.LevelDuration);
                        TrackDesignEvent($"Levels:Attempts:{levelFirstCompletion.LevelId}", levelFirstCompletion.Attempts);
                        break;
                    default:
                        var legacyFormat = analyticsEvent.ToGameAnalyticsFormat();
                        TrackDesignEvent(legacyFormat.Name, legacyFormat.Value);
                        break;
                }
            });
        }
