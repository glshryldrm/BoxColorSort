...
namespace HomaGames.Geryon
{
    ...
    public class GeryonConfigurationModel
    {
        ...
        {
            var geryonConfigurationModel = new GeryonConfigurationModel();
            ...
            if (deserializedJson.TryGetCanBeNull(E_GERYON_STATUS, out string status))
            {
                ...
                {
                    if (deserializedJson.TryGetNotNull(GERYON_RESULTS, out JsonObject result))
                    {
                        if (result.TryGetNotNull(STATUS, out string resultStatus))
                            geryonConfigurationModel.ResultStatus = resultStatus;

                        if (result.TryGetNotNull(MESSAGE, out string resultMessageCode))
            ...
            if (deserializedJson.TryGetNotNull(GERYON_RESULTS, out JsonObject geryonResults))
            {
                if (geryonResults != null && geryonResults.TryGetNotNull(RESULT_FIELD, out JsonObject resJson))
                {
                    if (resJson != null && resJson.TryGetNotNull(O_CONF, out JsonObject configuration))
                    {
                        geryonConfigurationModel.Configuration = configuration.ToRawData();

                        // Obtain IDs
                        resJson.TryGetNotNull<string>(S_SCOPE_ID,
                            id => geryonConfigurationModel.ScopeId = id);
                        resJson.TryGetNotNull<string>(S_VARIANT_ID,
                            id => geryonConfigurationModel.VariantId = id);
                        resJson.TryGetNotNull<string>(S_OVERRIDE_ID,
                            id => geryonConfigurationModel.OverrideId = id);

                        // Obtain external tokens
                        resJson.TryGetNotNull<string>(S_TOKEN_0,
                            token => geryonConfigurationModel.ExternalToken0 = token);
                        resJson.TryGetNotNull<string>(S_TOKEN_1,
                            token => geryonConfigurationModel.ExternalToken1 = token);
                        resJson.TryGetNotNull<string>(S_TOKEN_2,
                            token => geryonConfigurationModel.ExternalToken2 = token);
                        resJson.TryGetNotNull<string>(S_TOKEN_3,
                            token => geryonConfigurationModel.ExternalToken3 = token);
                        resJson.TryGetNotNull<string>(S_TOKEN_4,
