...
namespace HomaGames.HomaBelly
{
    public class EditorAnalyticsModelDeserializer
    {
        public EditorAnalyticsResponseModel Deserialize(string json)
        {
            ...
            inputObject.TryGetNotNull("ti", out responseModel.AppToken);
            inputObject.TryGetNotNull("ai", out responseModel.AppIdentifier);

            // Res dictionary
            if (inputObject.TryGetNotNull("res", out JsonObject resultObject))
            {
                resultObject.TryGetNotNull("s_event_name", out responseModel.EventName);
                
                // Gather all incoming "s_event_value_N"
                int valueIndex = 0;
                string eventValueKey = $"s_event_value_{valueIndex}";
                while (resultObject.TryGetCanBeNull(eventValueKey, out string eventValue))
                {
