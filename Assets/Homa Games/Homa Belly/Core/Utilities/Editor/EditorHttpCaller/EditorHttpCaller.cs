...
namespace HomaGames.HomaBelly.Utilities
{
    public class EditorHttpCaller<T>
    {
                ...
                using (HttpClient client = GetHttpClient())
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(errorString))
                        {
                            ...
                            // Detect any error
                            if (responseObject.TryGetCanBeNull("status", out string status) 
                                && responseObject.TryGetCanBeNull("message", out string message))
                            {
        ...
        public async Task<T> Get(string uri, IModelDeserializer<T> deserializer, CancellationToken cancellationToken = default)
        {
            try
            {
                using (HttpClient client = GetHttpClient())
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(errorString))
                        {
                            JsonObject responseObject = await Task.Run(() => Json.DeserializeObject(errorString), cancellationToken);
                            // Detect any error
                            if (responseObject.TryGetCanBeNull("status", out string status) 
                                && responseObject.TryGetCanBeNull("message", out string message))
                            {
                ...
                using (HttpClient client = GetHttpClient())
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(errorString))
                        {
                            ...
                            // Detect any error
                            if (resultObject.TryGetCanBeNull("status", out string status) 
                                && resultObject.TryGetCanBeNull("message", out string message))
                            {
                                throw new EditorHttpCallerException(status, message);
                            }
                        }
